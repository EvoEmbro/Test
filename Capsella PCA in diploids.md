...
rawTMM <- read.table("InputData/Diploids_individual_TMM_FLR.txt", header=TRUE, row.names=1)
head(rawTMM)[1:5]
dim(rawTMM)
...
#### remove genes with 0 expression value in any individual
```r
dim(rawTMM)
TMM <- na.omit(rawTMM)
dim(TMM)
...
#### Format the data the way of ggplot2 needs
```r
dim(pca.all$x)[1] # check how many PCs in total
pca.all.data <- data.frame(ID = 1:dim(pca.all$x)[1]) # create a new dataframe

for (i in 1:dim(pca.all$x)[1]){ # name each PCs
  pcs <- paste0("PC", i)
  pca.all.data[[pcs]] <- pca.all$x[,i]
}

# Set ID, Tissue, species, and matingSystem for ggplot
...
#### Output raw PCA plot
```r
ggplot(ZscoreAll, aes(PC1, PC2, col = Species, fill = Species))  +
...
#### We also want to know how this plot looks like when color by tissue, by species, and by mating system
```r
# Tissue
...
grid.newpage()
ggarrange(p1, p2, p3, ncol=3, nrow=1) # set the common legend
```

#### Causing all dots of flowers crowded together, I have to add zoom in plot for flower
The PCA analysis only for flower is need to run, where all steps are typically same as above mentioned but in different input data
...
#### Insert the flower plot into main figures.
```r
# by Tissue
p1 <- ggplot(ZscoreAll, aes(PC1, PC2, col = Tissue, fill = Species))  +
  geom_point(size = 6, stroke = 2, aes(shape = Species)) +
...
# graph parttern 2
# PC1 vs PC2
p1 <- ggplot(ZscoreFlower, aes(PC1, PC2, col = Species, fill = Species))  +
  geom_point(size = 6, stroke = 2, aes(shape = Species)) +
  scale_shape_manual(values=c(8,10,4))+
  scale_color_manual(values = c("#f90052","darkviolet","#006beb"))  +
  scale_fill_manual(values = c("#f90052","darkviolet","#006beb")) +
...
### Combine all tissues into one figure
